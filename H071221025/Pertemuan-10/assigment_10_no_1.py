...
class Two(Menu):
    def pilihan(self):
        if name:
          for i in range(len(name)):
            data = input("Silahkan Input Nama Baru: ")
            name[i] = data
        else:
            print("Data Saat ini Kosong")
...
class Five(Menu):
    def pilihan(self):
        ...
        while True:
            ...
            if re.fullmatch("^[a-z0-9]{1,}(@student\.unhas\.ac\.id$)", (input_email)):
        ...
        while True:
            ...
            if re.fullmatch(".{8,20}", (input_password)):
                if re.findall("[A-Z]+", (input_password)) and re.findall("[a-z]+", (input_password)) and re.findall("[0-9]+", (input_password)):
                    password.append(input_password)
                    break
                else:
                    print('=' * 50)
